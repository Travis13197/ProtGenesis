...
```
ProtGenesis article: https://www.biorxiv.org/content/10.64898/2026.02.20.706798v1
...
| Feature | Description | 特点描述 | 
|---------|-------------|------|
| **Protein Language Model-based embedding dataset extraction and standardization** | Supports **ProstT5**, **ProtT5** and other protein language model embeddings based on **customized python scripts(Jupyter Notebook)** | 支持 ProstT5、ProtT5 等面向蛋白质Genesis的蛋白质语言模型嵌入数据获取（**标准化Python脚本**）与蛋白质生成过程的标准化分析与生物学意义挖掘（**ProtGenesis** R包） |
| **Dimensionality Reduction, 2D & 3D visualization** | PCA, UMAP, tSNE and diverse visualizations (heatmap, scatterplot, 3d plot) for 1024D embedding dataset | 1024D 嵌入可视化的 PCA、UMAP、tSNE，包含2d和3d可视化 |
| **Tripartite Mathematical Metrics of Protein Structural Space** | **Local Density (ρ), Spatial Dispersion (D), Differential Embedding (δ)** | 三种分析蛋白质结构空间特征的数学指标：**局部密度(ρ)、空间离散度(D)、差分嵌入距离(δ)** |
| **New Visualization Modes** | **Assembly direction map**, **Genesis path map**, **Status transition map** | ✨提供三种ProtGenesis的新可视化方式：**组装方向图**、**起源路径图**、**状态转换图** |
...
<img src="https://github.com/Travis13197/ProtGenesis/blob/main/images/GraphicalAbstract.jpg" width="560px">
...
<details>
<summary> 3. Core Concepts </summary>
...
<details>
<summary> 5. Function Reference </summary>
...
<details>
<summary> 6. Data Structures </summary>
...
If you use ProtGenesis in your research, please cite | 如果您在研究中使用 ProtGenesis或者相关脚本用于分析，请引用：
...
Based on: Liu et al. (2026). Universal physical principles govern the deterministic genesis of protein structure. BioRxiv. https://doi.org/10.64898/2026.02.20.706798
