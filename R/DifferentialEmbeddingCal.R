...
#'   sequence = "G",
#'   seq_data = seurat_obj@assays$embed@layers$scale.data,
...
#'
#' @param seurat_obj Seurat object containing protein embeddings
...
#' @examples
#' \dontrun{
#' raw_embeddings <- GetEmbeddingData(seurat_obj, "raw")
#' pca_embeddings <- GetEmbeddingData(seurat_obj, "pca", pca_npcs = 50)
#' }
GetEmbeddingData <- function(seurat_obj, embedding_type = c("raw", "pca", "tsne", "umap"), 
  ...
  switch(embedding_type,
         "raw" = {
           if ("embed" %in% names(seurat_obj@assays)) {
             if ("scale.data" %in% names(seurat_obj@assays$embed@layers)) {
               embeddings <- t(seurat_obj@assays$embed@layers$scale.data)
             } else if ("data" %in% names(seurat_obj@assays$embed@layers)) {
               embeddings <- t(seurat_obj@assays$embed@layers$data)
             } else {
         ...
         "pca" = {
           if ("pca" %in% names(seurat_obj@reductions)) {
             npcs_use <- min(pca_npcs, ncol(Embeddings(seurat_obj, "pca")))
             embeddings <- Embeddings(seurat_obj, "pca")[, 1:npcs_use, drop = FALSE]
           } else {
         ...
         "tsne" = {
           if ("tsne" %in% names(seurat_obj@reductions)) {
             embeddings <- Embeddings(seurat_obj, "tsne")
           } else {
...
#' )
#' }
CalculateDifferentialEmbedding <- function(seurat_obj,
                                           target_sequences,
  ...
  if (add_amino_acid_sequence && !"Amino_Acid_Sequence" %in% colnames(seurat_obj@meta.data)) {
    if (verbose) {
      cat("添加氨基酸序列到meta.data...\n")
    }
    seurat_obj@meta.data$Amino_Acid_Sequence <- sapply(
      seurat_obj@meta.data$Cell,
  ...
  if (!is.null(split_by)) {
    ...
    if (is.null(split_values)) {
      split_values <- unique(seurat_obj@meta.data[[split_by]])
    }
  ...
  embeddings <- GetEmbeddingData(seurat_obj, embedding_type, pca_npcs, verbose)
  ...
  process_split <- function(split_value = NULL) {
    ...
    meta_subset <- seurat_obj@meta.data
...
SaveDifferentialEmbeddingResults <- function(result,
  ...
  save(combined_df, differential_df, seurat_obj, file = rdata_file)
